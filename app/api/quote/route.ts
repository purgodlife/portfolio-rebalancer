 ...
 * 직접 호출하면 CORS에 막히기 때문에, 이 서버리스 함수가 "티커 심볼만" 그대로
 * 전달해 시세를 대신 받아오는 순수 중계(pass-through) 역할만 한다.
 *
 * 이 함수를 통과하는 데이터는 오직 요청 쿼리의 "symbol" 문자열뿐이다.
 * 사용자의 포트폴리오 데이터(보유종목/수량/매수가/자산배분 등)는 이 함수에
 * 절대 전달되지 않고, 요청/응답 내용을 별도로 저장하거나 로깅하지도 않는다
 * (Vercel의 기본 액세스 로그 이외에는 아무 기록도 남기지 않음).
 */
